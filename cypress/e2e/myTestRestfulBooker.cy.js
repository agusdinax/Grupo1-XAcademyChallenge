describe("3.1 - Navegacion Habitaciones - Shady Meadows B&B", () => {
  ...
  it("CP-003 | Ver formulario de reserva", () => {
  ...
  it("CP-004 | Navegar entre habitaciones", () => {
    // Selecciona la habitacion
    // Busca el botón "Book now" dentro de la tarjeta de habitación
    cy.eligeHabitacion();
...
describe("3.1 - Reservas - Usuario Invitado - Shady Meadows B&B", () => {
  ...
  it("CP-007 | Realizar una reserva con datos válidos", function () {
    ...
    cy.completarFormularioReserva(this.huespedes.huesped_1);
  ...
  it("CP-008 | Realizar dos reservas consecutivas con datos válidos", function () {
    ...
    cy.completarFormularioReserva(this.huespedes.huesped_2);
...
describe("3.2 Validaciones del Formulario de Reserva - Shady Meadows B&B", () => {
  const datosValidos = {
    nombre: "Juan",
    apellido: "Perez",
  ...
  it("CP-013 | Nombre vacío - debe mostrar error Firstname should not be blank", function () {
    cy.completarFormularioReserva(this.datosReserva.nombreVacio);
  ...
  it("CP-014 | Apellido vacío - debe mostrar error Lastname should not be blank", function () {
    cy.completarFormularioReserva(this.datosReserva.apellidoVacio);
    cy.enviarFormularioReserva();
  ...
  it("CP-015 | Email vacío - debe mostrar error must not be empty", function () {
    cy.completarFormularioReserva(this.datosReserva.correoVacio);
    cy.enviarFormularioReserva();
  ...
  it("CP-015b | Email mal formado - debe mostrar error must be a well-formed email address", function () {
    cy.completarFormularioReserva(this.datosReserva.correoInvalido);
    cy.enviarFormularioReserva();
  ...
  it("CP-016 | Teléfono vacío - debe mostrar error must not be empty", function () {
    cy.completarFormularioReserva(this.datosReserva.telefonoVacio);
    cy.enviarFormularioReserva();
  ...
  it("CP-020 | Formulario vacío - deben aparecer todos los mensajes de error", () => {
    cy.enviarFormularioReserva();
    cy.get('.alert-danger, .error, [class*="error"], [class*="alert"]').should(
      "be.visible",
    );
    cy.get("body")
...
describe("3.3 Formulario de Contacto - Shady Meadows B&B", () => {
  ...
  it("CP-025 | Mensaje de longitud máxima", () => {
    cy.intercept("POST", "/api/message").as("contactFail");
  ...
  it("CP-026 | Caracteres especiales", () => {
    cy.intercept("POST", "/api/message").as("contactSuccess");
